...
import GenresModel from '../src/models/GenresModel';
import MovieModel from '../src/models/MovieModel';
import UserModel from '../src/models/UserModel';

const fakeData = { type: 'Fake' };
const fakeMovie = {
  Title: 'Riverdale',
  Year: '2017–',
  Rated: 'TV-14',
...
it('Genre interface returns type: Fake', () => {
  const model = new GenresModel(fakeData);
  expect(model).toEqual({ type: 'Fake' });
});

it('Genre interface returns type: empyt ', () => {
  const model = new GenresModel();
  expect(model).toEqual({ type: '' });
});

it('Movie interface returns type: fakeMovie', () => {
  const model = new MovieModel(fakeMovie);
  expect(model).toMatchObject(fakeMovie);
});

it('Movie interface returns type: fakeMovieEmpty ', () => {
  const model = new MovieModel();
  expect(model).toMatchObject(fakeMovieEmpty);
});

it('User interface returns type: Fake', () => {
  const model = new UserModel(fakeUser);
  expect(model).toMatchObject(fakeUser);
});

it('User interface returns type: empyt ', () => {
  const model = new UserModel();
  expect(model).toMatchObject(fakeUserEmpty);
});
